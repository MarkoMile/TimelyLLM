...
from .minispec_interpreter import MiniSpecInterpreter, MiniSpecProgram
...
class LLMController():
    def __init__(self, robot_info_list: list[RobotInfo], message_queue: Optional[queue.Queue]=None):
        ...
        self.program = MiniSpecProgram(self.robots, self.message_queue)
        self.agent_id = robot_info_list[0].robot_id
    ...
    def execute_task(self, json_output: Stream | str, eval_flag: bool=False, task_id: int=0):
        # stream_interpreting = False if isinstance(json_output, str) else True
        self.program.parse(json_output, False)
        if eval_flag:
            print_t(f"[M] Start executing task {task_id} for agent {self.agent_id}")
            thread = threading.Thread(target=lambda task_id=task_id: self.program.eval(task_id))
            thread.start()
