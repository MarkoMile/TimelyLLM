...
class Go2Observation(RobotObservation):
    ...
    def init_ros_observation(self):
        print_t(f"[ROS] init_ros_observation called")
        ...
        def _ros_image_callback(image: msg.Image):
            # print_t(f"[ROS] Image callback triggered")
            # Convert RGB to BGR
            ...
            # save_path = os.path.join(CURRENT_DIR, f"go2_ros_{timestamp}.jpg")
            # self._image.save(save_path)
            # print_t(f"[Saved] Image saved to {save_path}")
    ...
    def _start(self):
        if self.ros:
            ...
            print_t(f"[ROS] ros_thread started")
        else:
    ...
    async def process_image(self, image: Image.Image):
        ...
        # print("[Check] robot dog go2 process_image() called successfully.")
        # try:
        #     await self.yolo_client.detect(image)
        #     print("[Check] detect() called successfully.")
...
class Go2Wrapper(RobotWrapper):
    def __init__(self, robot_info: RobotInfo, system_skill_func: list[callable]):
        ...
        if self.ros:
            ...
            assert self.node is not None, "Node creation failed"
            assert self.control_publisher is not None, "Publisher creation failed"
        else:
        ...
        high_level_skills = [
            ...
            {
                "name": "orienting",
                "definition": "{_1=ox($1);rotate((0.5-_1)*60)}",
    ...
    def rotate(self, deg: float) -> tuple[bool, bool]:
        ...
        # if abs(deg) != 45:
        #     cv2.imwrite(f"debug_rotate_{time.time()}.jpg", np.array(self.observation._image))
