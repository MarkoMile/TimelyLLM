...
class MiniSpecProgram:
    ...
    def parse(self, json_output: Stream | str, stream_interpreting: bool=False) -> bool:
        # print_t(f'Parsing output: {json_output} Parse state: {self.parse_state}, Buffer: {self.parse_buffer}')
    ...
    def eval(self, task_id:int) -> MiniSpecReturnValue:
        return self.statement.eval(task_id)
...
class Statement:
    def __init__(self, env: dict, robot: RobotWrapper):
        ...
        robot_id = robot.robot_info.robot_id
        ...
        for c in code:
            ...
            match self.action:
                ...
                case CodeAction.SEQ:
                    ...
                    # print(f'$$$ {c} {executable} {self.current_statement.executable}')
                    if not executable and self.current_statement.executable:
                        self.executable = True
                        # print(f'### Statement executable: {self.current_statement.action}')
                        self.sub_statements.append(self.current_statement)
                        # print(f"[Debug] Append sub_statements at {hex(id(self.sub_statements))}")
                        # print(f"[Debug] Append sub_statements at {hex(id(self))}")
    ...
    def eval(self, task_id = None) -> MiniSpecReturnValue:
        # _print_debug(f'Eval statement: {self.to_string_simple()}')
        # if task_id is not None:
        # print_t(f"[Agent {self.agent_id}] Task id: {task_id} Eval statement: {self.to_string_simple()} Executable: {self.executable}")
        ...
        match self.action:
            case CodeAction.ATOMIC:
                ...
                if task_id is None:
                    # print_t(f"[Agent {self.agent_id}] Executing atomic statement: {self.sub_statements[0]}")
                    return self.eval_expr(self.sub_statements[0])
                else:
                    # print_t(f"[Agent {self.agent_id}] Task id: {task_id} Executing atomic statement: {self.sub_statements[0]}")
                    start_time = time.time()
                    self.log_skill_start(self.to_string_simple(), task_id, start_time)
                    result = self.eval_expr(self.sub_statements[0])
                    end_time = time.time()
                    self.log_skill_end(self.to_string_simple(), task_id, end_time)
                    return result

            case CodeAction.SEQ:
                for statement in self.sub_statements:
                    ret_val = statement.eval(task_id)
            ...
            case CodeAction.IF:
                for i in range(len(self.condition)):
                    if task_id is None:
                        ...
                        end_time = time.time()
                        self.log_skill_end(self.to_string_simple(), task_id, end_time)
                        # print_t(f"[Agent {self.agent_id}] Task id: {task_id} Evaluating condition: {self.condition[i]} = {condition_val.value}")

                    if condition_val.replan:
                        return condition_val
                    
                    if condition_val.value == True:
                        while len(self.sub_statements)==0:
                            time.sleep(0.1)
                            # print(f"[Debug] Waiting: sub_statements is empty at {hex(id(self.sub_statements))}")
                            # print(f"[Debug] Waiting: sub_statements is empty at {hex(id(self))}")
                            # print_t(f"[Agent {self.agent_id}] Task id: {task_id} Waiting for sub-statements to be ready...")
                        ret_val = self.sub_statements[i].eval(task_id)
                        if self.sub_statements[i].ret:
                            self.ret = True
                        return ret_val
                
                if len(self.sub_statements) > len(self.condition):
                    ret_val = self.sub_statements[-1].eval(task_id)
            ...
            case CodeAction.LOOP:
                ...
                for i in range(self.loop_count):
                    ret_val = self.sub_statements[0].eval(task_id)
    ...
    def log_skill_start(self, action_name: str, task_id, start_time: float):
        logger.info(
            f"Start executing task {task_id} for agent {self.agent_id} on time {start_time} with plan {action_name}"
        )

    def log_skill_end(self, action_name: str, task_id, end_time: float):
        logger.info(
            f"Finish executing task {task_id} for agent {self.agent_id} on time {end_time} with plan {action_name}"
        )
