...
def generate_random_data_structures(num_traces, max_trace_per_unit=None):
    ...
    structure_len = np.random.randint(50, 101)

    structure = []
    for _ in range(structure_len):
        n = np.random.randint(max_trace_per_unit/2, max_trace_per_unit+1)   
...
if __name__ == "__main__":
    ...
    # Parameters
    # max_num_per_event = 8
    # lambda_rate = 0.25 # event per second
    # job_interval = 0.001

    # for drone-typefly
    # output_file= './dataset/data_sample_13.json'
    # output_file2= './dataset/data_sample_13_config.json'
    # input_file = './dataset/trace_set_3.json'

    # for robotarm-fltrnn
    # max_num_per_event = 12
    # lambda_rate = 0.1 # event per second
    # job_interval = 0.001
    # output_file= './dataset/data_sample_robotarm_fltrnn.json'
    # output_file2= './dataset/data_sample_robotarm_fltrnn_config.json'
    # input_file = './dataset/trace_set_robotarm_fltrnn.json'

    # for mixed robots: robotdog and drone
    max_num_per_event = 16
    lambda_rate = 0.25 # event per second
    job_interval = 0.001
    output_file= './dataset/data_sample_mixed_robots.json'
    output_file2= './dataset/data_sample_mixed_robots_config.json'
    input_file = './dataset/trace_set_robotdog_drone.json'
    ...
    for event_id, arrival_time in enumerate(task_arrivals):
        ...
        for trace_idx in data_sample_structure[event_id]:
            ...
            runtime = task['task_exe_time']
            plantime = task['plan_time']
            end_time = start_time + runtime + plantime + 1 # add 1s to guarentee time interval between two tasks
            ...
            ordered_task = {
                ...
                "robot_system": task['robot_system'],
